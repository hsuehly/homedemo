...
function useCountDown(
  ...
  useEffect(() => {
    ...
    if ((currentTime > 0 && endTime > 0) || differTime > 0) {
      entryTime.current = new Date().getTime(); // 缓存当前时间
      maxTime.current = differTime > 0 ? differTime : endTime - currentTime; // 时间差值 有相差的使用相差的没有结束和开始的相差时间
      if (maxTime.current <= 0 && noImmediate) {
        isImplementCb.current = false;
      }
      setDiffTime(maxTime.current); // 更新相差的时间
    }
